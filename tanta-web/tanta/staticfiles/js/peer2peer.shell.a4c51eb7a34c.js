p2p.shell = (function () {
	...
	var
		configMap = {
			...
			main_html: String()
			+'<h2 class="center">Peer 2 Peer Summary</h2>'
			...
			+'<ul class="list-reset mt0">'
				+'<li class="inline-block mr1"><h4 class="view borrow center" style="background: #fafafa; cursor: pointer;" id="view_type">Borrow</h3></li>'
				+'<li class="inline-block mr1"><h4 class="view lend center" style="background: #fafafa; cursor: pointer;" id="view_type">Lend</h3></li>'
			+'</ul>'
			+'</div>'
			...
			+'<h3 class="group mb0 pb0">Click the tabs to see your lending information</h3>'
			+'<ul class="list-reset flex justify-center mt0" id="info">'
			+'</ul>'
			+'</div>'
		},
		stateMap = {
	...
	toggleTab = function (selected) {
		var info_container = jqueryMap.$container.find('#info');
		var group = jqueryMap.$container.find('.group');
		group.empty();
		info_container.html('<li class="mx-auto"><h4 class="mx2">Loan ID</h4><div class="item-header1"></div></li>'
				+'<li class="item-header mx-auto"><h4 class="type mx2"></h4><div class="item-header2"></div></li>'
				+'<li class="item-header mx-auto"><h4 class="mx2">Loan Amount</h4><div class="item-header3"></div></li>'
				+'<li class="item-header mx-auto"><h4 class="mx2">Start Date</h3><div class="item-header4"></div></li>');
			p2p.summary.configModule({});
			p2p.summary.initModule(info_container.find(".item-header1"));
			jqueryMap.$tab.css({
					...
					"z-index": "-1"});
		if (selected) {
			info_container.find(".type").text("Lender")
			...
			p2p.summary.getBorrow(info_container,changeBorrow.html());
		}
			else {
					info_container.find(".type").text("Borrower")
					var changeLend = jqueryMap.$container.find('.view.lend');
					changeLend.css({background: "#f48668","box-shadow": "5px 0px 10px #888888", 
					width: "100px","z-index": 1});				
				p2p.summary.getBorrow(info_container,changeLend.html());
